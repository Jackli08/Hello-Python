...
base_url = 'https://www.spiderbuf.cn/playground/s06'
header = {
...
url = 'https://www.spiderbuf.cn' + str(re.findall('<iframe src="(.*?)"',html)[0])
...
for tr in trs:
    ...
    s = ''
    for td in tds:
        string = str(td.xpath('string(.)'))
        # print(string.replace(' ',''))
        s = s + string.replace(' ','').replace('\n','') + '|'
    if s != '':
        f.write(s + '\n')
f.close() 
